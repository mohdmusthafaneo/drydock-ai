...
| **Move `tmp/*.py` one-offs into the product** | Promote the Sprint-Evidence pipeline into `src/lib/evidence/` + `ml.*` jobs per RFC §5.1. | `tmp/connexus-sprint34-*` | Turns a proven one-off into a maintained feature; removes secret-leak-prone scratch scripts from the tree. |
