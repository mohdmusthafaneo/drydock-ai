...
const persistQAReportTool = createTool({
  ...
  execute: async (inputData, context) => {
    ...
    const normalizedStatusBuckets = QA_PRESET_VALUES.map((preset) => ({ preset, count: statusMap[preset] }));
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const statusRows = QA_PRESET_VALUES.map((preset) => ({
        runId,
        organizationId,
        preset,
