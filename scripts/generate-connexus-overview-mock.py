#!/usr/bin/env python3
"""Generate src/lib/store/mock/connexus-overview-derived.ts from a Connexus Jira CSV.

Usage:
  python3 scripts/generate-connexus-overview-mock.py [path/to/cnxs.csv]

Defaults to fixtures/jira/cnxs.csv.
"""
...
OUT_TS = ROOT / "src/lib/store/mock/connexus-overview-derived.ts"
DEFAULT_CSV = ROOT / "fixtures/jira/cnxs.csv"

# Single demo team — CSV has no usable Team Name / Team field for Overview.
TEAM_KEY = "CONNEXUS"
TEAM_NAME = "Connexus"
TEAM_KEY_TO_NAME = {TEAM_KEY: TEAM_NAME}

SCOPE_TYPES = {"Story", "Bug", "Feature", "Epic"}
# Invented ~2-week windows ending near CSV activity (as-of 2026-09-09).
SPRINTS = [
    {"id": "37", "name": "Sprint 37", "start": date(2026, 8, 27), "end": date(2026, 9, 9), "active": True},
    {"id": "36", "name": "Sprint 36", "start": date(2026, 8, 13), "end": date(2026, 8, 26), "active": False},
    {"id": "35", "name": "Sprint 35", "start": date(2026, 7, 30), "end": date(2026, 8, 12), "active": False},
    {"id": "34", "name": "Sprint 34", "start": date(2026, 7, 16), "end": date(2026, 7, 29), "active": False},
]
SPRINT_BY_ID = {s["id"]: s for s in SPRINTS}
AS_OF = datetime(2026, 9, 9, 16, 48, 0)
AS_OF_DATE = AS_OF.date()
PREV_MAP = {"37": "36", "36": "35", "35": "34", "34": None}
DEFAULT_SPRINT_ID = "37"
ORG_NAME = "Connexus"
PROJECT_KEY = "CX"
JIRA_SITE_URL = "https://neoito-team-connexus.atlassian.net"
...
def parse_version_date(name: str):
    """Best-effort target date from fix-version names."""
...
def main():
    ...
    def team_matches(p, team_key):
        return team_key is None or p["team"] == team_key
    ...
    def compute_kpis(sprint_id, team_key=None, prev_sprint_id=None):
        ...
        ai, comp = (5, 2) if team_key else (5, 2)
        team_label = TEAM_KEY_TO_NAME.get(team_key, ORG_NAME)
        name = sprint["name"]
    ...
    org37 = compute_kpis(DEFAULT_SPRINT_ID, None, PREV_MAP[DEFAULT_SPRINT_ID])
    historical = [s["id"] for s in SPRINTS if s["id"] != DEFAULT_SPRINT_ID]
    payload = {
        "lastSyncAt": AS_OF.strftime("%Y-%m-%dT%H:%M:%S.000Z"),
        "orgName": ORG_NAME,
        "projectKey": PROJECT_KEY,
        "defaultSprintId": DEFAULT_SPRINT_ID,
        ...
        "base": {
            **org37,
            "charts": {
                "deliveryTrend": build_trend(DEFAULT_SPRINT_ID),
                "burndown": build_burndown(DEFAULT_SPRINT_ID),
                "heatmap": build_heatmap(DEFAULT_SPRINT_ID),
            },
        },
        "byTeam": {tk: compute_kpis(DEFAULT_SPRINT_ID, tk, PREV_MAP[DEFAULT_SPRINT_ID]) for tk in TEAM_KEY_TO_NAME},
        "bySprint": {
            sid: {
        ...
        "notes": {
            "jiraSource": str(csv_path.relative_to(ROOT) if csv_path.is_relative_to(ROOT) else csv_path),
    ...
    OUT_TS.write_text(
        f"""/**
 * Overview mock derived from Connexus Jira export (`{csv_path}`).
 ...
 */
export const CONNEXUS_OVERVIEW_DERIVED = {json.dumps(payload, indent=2)} as const;
...
"""
    ...
    print(
        f"base score={org37['score']} {org37['band']} "
        f"{org37['done']}/{org37['total']} blocked={org37['blocked']} spill={org37['spillover']}"
    )
