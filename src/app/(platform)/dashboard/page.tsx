...
import { Phase1FoundationCard } from "@/components/enterprise/phase1-foundation-card";
import {
  OperationalTimeline,
  type TimelineItem,
...
export default async function EnterpriseDashboardPage() {
  ...
  const kpis = [
    ...
    { label: "Integrations healthy", value: `${ctx.stats.integrationsHealthy}/${ctx.stats.connectedTools}`, icon: Activity },
    { label: "Pending approvals", value: String(ctx.stats.pendingApprovals), icon: CheckCircle2 },
  ];

  const phase1Checks = [
    { id: "dna", label: "Governance & Delivery DNA configured", done: Boolean(ctx.dna), href: "/governance/setup" },
    { id: "integrations", label: "Critical integrations connected", done: ctx.stats.connectedTools >= 2, href: "/integrations" },
    { id: "telemetry", label: "Telemetry pipeline active", done: ctx.stats.telemetryEventCount > 0, href: "/observability" },
    { id: "approvals", label: "Approval workflow ready", done: Boolean(ctx.governancePolicy), href: "/approvals" },
    { id: "audit", label: "Audit logging enabled", done: ctx.stats.auditEventCount > 0, href: "/audit" },
  ];

  const recentEvents = ctx.events.slice(0, 6);

  const timeline: TimelineItem[] = [
    ...ctx.telemetryEvents.slice(0, 4).map((e) => ({
      id: e.id,
      type: "telemetry.ingested",
      title: `${e.eventType} · ${e.source}`,
      description: e.service ?? e.environment ?? undefined,
      at: e.occurredAt,
    })),
    ...
    <div className="w-full space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-secondary">
            {org?.name} · {ctx.dna.autonomyMode} mode · governance score{" "}
            <span className="font-medium text-primary">{ctx.stats.governanceScore}/100</span>
          </p>
        </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold">Deep observability</h2>
