...
import {
  previousSprintId,
  type OverviewDerivedPack,
} from "@/lib/store/mock/overview-derived";
import { resolveMockScheduleRiskFromDerived } from "@/lib/store/mock/schedule-risk";
...
function createDeliveryBuilders(derived: OverviewDerivedPack) {
  const teams = derived.teams;
  const projectKey = derived.projectKey;
  const orgName = derived.orgName;
  const jiraSiteUrl = derived.jiraSiteUrl;
  const lastSyncAt = derived.lastSyncAt;

  function sprintKpis(sprintId: string): DerivedKpis {
    if (sprintId === derived.defaultSprintId) {
      return cloneKpis(derived.base);
    }
    const entry = derived.bySprint[sprintId];
    return cloneKpis(entry?.kpis ?? derived.base);
  }

  function teamKpis(teamKey: string, sprintId: string): DerivedKpis {
    if (sprintId === derived.defaultSprintId) {
      const row = derived.byTeam[teamKey];
      if (row) return cloneKpis(row);
    }
    const row = derived.byTeamSprint[`${teamKey}:${sprintId}`];
  ...
  function deliveryExtras(
    ...
    if (teamKey) {
      const scoped = derived.deliveryByTeamSprint[`${teamKey}:${sprintId}`];
      if (scoped) return scoped as DerivedDeliveryExtras;
    }
    return derived.deliveryBySprint[sprintId] as DerivedDeliveryExtras;
  }
  ...
  function buildSignals(input: {
    ...
    signals.push({
      ...
      severity:
        kpis.bugs >= 15 ? "critical" : kpis.bugs >= 5 ? "warning" : "info",
    });
  ...
  function sprintState(sprintId: string): DeliveryAnalysisSprintRow["state"] {
    const sprint = derived.sprints.find((s) => s.id === sprintId);
    if (!sprint) return "future";
    if (sprintId === derived.defaultSprintId) return "active";
    const current = derived.sprints.find(
      (s) => s.id === derived.defaultSprintId,
    );
    if (current && toDateKey(sprint.end) < toDateKey(current.start)) {
      return "closed";
    }
    return "future";
  }
  ...
  function trendForSprint(sprintId: string, teamKey?: string | null) {
    ...
    const window = (
      idx >= 0 ? ordered.slice(idx, idx + 3) : ordered.slice(0, 3)
    ).reverse();
    return window.map((id) => {
      const kpis = teamKey ? teamKpis(teamKey, id) : sprintKpis(id);
      const extras = deliveryExtras(id, teamKey);
      const sprint = derived.sprints.find((s) => s.id === id);
      return {
        syncedAt: sprint ? `${sprint.end}T10:00:00.000Z` : lastSyncAt,
  ...
  function buildSnapshot(
    ...
    const prevId = previousSprintId(derived, sid);
    ...
    const sprintActive = sid === derived.defaultSprintId;
    ...
    const scheduleRisk = resolveMockScheduleRiskFromDerived(
      derived,
      sid,
      teamKey ?? null,
      { siteUrl: jiraSiteUrl },
    ...
    return {
      ...
      kpis: {
        ...
        overdue: extras.overdue,
        overdueDelta: prevExtras
          ? extras.overdue - prevExtras.overdue
          : undefined,
        spillover: kpis.spillover,
        spilloverDelta: prevKpis
          ? kpis.spillover - prevKpis.spillover
          : undefined,
        bugsOpen: kpis.bugs,
  ...
  function buildAnalysis(): DeliveryAnalysisData {
    const defaultId = derived.defaultSprintId;
    ...
    for (const team of teams) {
      byTeam[team.key] = buildSnapshot(defaultId, team.key);
    }
    for (const sprint of derived.sprints) {
      bySprint[sprint.id] = buildSnapshot(sprint.id);
    }
    for (const team of teams) {
      for (const sprint of derived.sprints) {
        byTeamSprint[teamSprintKey(team.key, sprint.id)] = buildSnapshot(
          sprint.id,
          team.key,
        );
      }
  ...
  return { buildSnapshot, buildAnalysis };
}

/**
 * Sprint/team-aware Delivery snapshot from a Jira-derived pack.
 */
export function buildMockDeliverySnapshotFromDerived(
  derived: OverviewDerivedPack,
  sprintId?: string | null,
  teamKey?: string | null,
): DeliveryAnalysisSnapshot {
  return createDeliveryBuilders(derived).buildSnapshot(sprintId, teamKey);
}

export function buildMockDeliveryAnalysisFromDerived(
  derived: OverviewDerivedPack,
): DeliveryAnalysisData {
  return createDeliveryBuilders(derived).buildAnalysis();
}

/** @deprecated Prefer pack-aware builders; kept for TPT call sites. */
export function buildMockDeliverySnapshot(
  sprintId?: string | null,
  teamKey?: string | null,
): DeliveryAnalysisSnapshot {
  return buildMockDeliverySnapshotFromDerived(
...
export function buildMockDeliveryAnalysis(): DeliveryAnalysisData {
  return buildMockDeliveryAnalysisFromDerived(
    TPT_OVERVIEW_DERIVED as unknown as OverviewDerivedPack,
  );
}
