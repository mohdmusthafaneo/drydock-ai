...
export function EngineeringDetailSection({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className={cn("space-y-3", className)}>
      <CollapsibleTrigger className="group flex w-full items-center justify-between gap-3 rounded-[var(--radius-card)] border border-border bg-pure-white px-5 py-4 text-left shadow-[var(--shadow)] transition-colors hover:bg-fog/60">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <p className="font-display text-[17px] leading-snug text-ink">{title}</p>
            {typeof count === "number" ? (
              <span className="inline-flex items-center rounded-[8px] border border-dove/50 bg-fog px-2 py-0.5 text-[11px] font-medium text-ash">
                {count}
              </span>
            ) : null}
          </div>
