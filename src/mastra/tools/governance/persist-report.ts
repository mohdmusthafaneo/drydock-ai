...
export const persistGovernanceReportTool = createTool({
  id: "persist-governance-report",
  description:
    "Persist a Governance analysis run (repowise risk + health + dead-code) into tenant-scoped normalized Prisma tables.",
  ...
  toModelOutput: (output) => {
    if (!output || typeof output !== "object" || !("runId" in output)) {
      return { type: "text" as const, value: "Persisting governance analysis..." };
    }
  ...
  execute: async (inputData, context) => {
    ...
    const headlineWorstFileScore =
      worstFiles[0]?.score === undefined || worstFiles[0]?.score === null
        ? null
        : Number(worstFiles[0].score);
    ...
    const worstFilesNorm = worstFiles
      .map((w) => ({
    ...
    const deadCodeNorm = deadCodeFindings
      .map((d) => ({
        kind: d.kind ?? null,
    ...
    const kpisStable = Object.keys(kpis)
      .sort()
      .map((kpiKey) => {
        const v = kpis[kpiKey];
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const kpiRows = Object.keys(kpisStable).length
        ? kpisStable.map((pair) => {
            const [kpiKey, norm] = pair as any;
            return {
              runId,
              organizationId,
              kpiKey,
              valueFloat: (norm.f as number | null) ?? null,
              valueString: norm.s as string | null,
            };
          })
        : [];
    ...
    return {
      ...
      rowsPersisted:
        result.rowCounts.kpis +
        result.rowCounts.worstFiles +
        result.rowCounts.riskDrivers +
        result.rowCounts.healthFindings +
        result.rowCounts.deadCodeFindings,
    };
  },
});
