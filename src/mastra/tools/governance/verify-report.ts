...
export const verifyGovernanceReportTool = createTool({
  id: "verify-governance-report",
  description:
    "Re-query a persisted GovernanceAnalysisRun and validate normalized row integrity. Marks the run VERIFIED or FAILED.",
  ...
  toModelOutput: (output) => {
    ...
    return {
      type: "text" as const,
      value: `Governance verification ${output.ok ? "passed" : "failed"} for runId=${output.runId} (${
        output.status
