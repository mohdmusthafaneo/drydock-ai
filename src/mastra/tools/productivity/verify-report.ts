...
export const verifyProductivityReportTool = createTool({
  ...
  description:
    "Re-query a persisted ProductivityAnalysisRun and validate normalized row integrity against the analyzer's headline totals. Marks the run VERIFIED or FAILED.",
  ...
  execute: async (inputData, context) => {
    ...
    const addCheck = (name: string, expected: unknown, actual: unknown, pass: boolean) => {
      checks.push({ name, expected, actual, pass });
    };

    /** Allow tiny analyzer/timezone drift on large repos (absolute or 0.5%). */
    const approxEqual = (expected: number, actual: number): boolean => {
      if (expected === actual) return true;
      const tol = Math.max(20, Math.ceil(Math.abs(expected) * 0.005));
